...
\subsection{预测流程}
整个预测过程可以概括为以下流程（见图\ref{fig:flow}）：
\[
\text{文物成分数据} 
\;\xrightarrow{\text{标准化}}\; 
\text{决策树大类预测}
\;\xrightarrow{\text{K-means聚类}}\;
\text{亚类划分}
\]
...
\tikzstyle{process} = [rectangle, rounded corners=6pt, draw=black, fill=blue!10, text centered, minimum height=1cm, minimum width=3cm]
\tikzstyle{arrow} = [->, thick]

\node (input) [process] {表单3文物成分数据};
\node (norm) [process, below of=input] {标准化处理};
\node (tree) [process, below of=norm] {决策树模型：大类预测（高钾 / 铅钡）};
\node (kmeans) [process, below of=tree] {K-means 聚类：亚类划分};
\node (output) [process, below of=kmeans] {预测结果输出（类别 + 亚类）};

\draw [arrow] (input) -- (norm);
\draw [arrow] (norm) -- (tree);
\draw [arrow] (tree) -- (kmeans);
...
\subsection{预测结果与敏感性分析}
将表单3的未知样本逐一代入上述流程后，可以得到每个文物的类别与亚类结果，预测结果如表\ref{tab:pred} 所示。
